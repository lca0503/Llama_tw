...
def main(
    ...
    for idx in range(0, len(prompts), batch_size):
        ...
        all_sequences_without_prompt += [x.outputs[0].token_ids for x in ret]

    # Save results
    os.makedirs(f"{output_dir}/{model_name}/", exist_ok=True)

    prompt_lang = data_path.split(".")[-1]
    with open(f"{output_dir}/{model_name}/lid-{prompt_lang}.pkl", "wb") as fp:
        pickle.dump(all_sequences_without_prompt, fp)
